# -*- coding: utf-8 -*-
"""equity_cuve_STOXX.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1JpMEUZBS3akvgeT2CFS3tHNgjBFRon64
"""
...
def stat_arb_success(stock_test, net):
    ...
    for i in range(int(len(stock_test[0])/10)):
        ...
        for k in range(z):
          ...
          positions[k].append(delta.tolist())
...
for trial in range(trials):
  ...
  for epoch in range(num_epochs):
      ...
      if epoch%10==0:
        ...
        net.eval()
        record, positions = stat_arb_success(stock_test, net)
        net.train()
        print(record)
...
def equity_curve(stock_test, delta_list):
  profit_list = []
  cum_profit = 0
  for i in range(len(delta_list[0])):
    for k in range(z):
          delta = np.array(delta_list[k][i])
          gain = np.dot(delta, stock_test[k][10*i+1:10*i+10]-stock_test[k][10*i:10*i+9])
          cost = trans_cost * np.abs(np.concatenate([np.array([delta[0]]), delta[1:]-delta[:-1], np.array([delta[-1]])])).sum()
          cost += 0.5 * bid_ask_spread * np.abs(np.concatenate([np.array([delta[0]]), delta[1:]-delta[:-1], np.array([delta[-1]])])).sum()
          cost += (borrowing_cost/252.0*np.maximum(-1*delta, 0)*stock_test[k][10*i:10*i+9]).sum()
          cum_profit += (gain-cost)
    profit_list.append(cum_profit)
  return profit_list

ranking = list(enumerate(item[0]['Average'] for item in pnl))

ranking = sorted(ranking, key = lambda x:x[1])

ranking

quantiles = [ranking[i][0] for i in [0,10,20,30,40,49]]

sns.set()
legends = ["0%", "20%", "40%", "60%", "80%", "100%"]
for count, i in enumerate(quantiles):
  plt.plot(equity_curve(stock_test, pnl[i][1]), label = legends[count])
plt.axhline(y=0, color='r', linestyle='--')
plt.xlabel("Time (trading periods)")
plt.ylabel("Net Profits")
plt.legend()
plt.savefig('equity_curve_STOXX.pdf', bbox_inches='tight')



for i in range(z):
  plt.plot(stock_test[i])
plt.show()
