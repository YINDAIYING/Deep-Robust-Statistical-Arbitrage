...
def get_frac_vector(P_tau_tilde, indicator_matrix):
    ...
    for i in range(z):
      delta = delta_list[i]
      h_value_trans += trans_cost * torch.abs(P_tau_tilde[:,10*i:10*i+10] * torch.cat((delta[:,:1], delta[:,1:]-delta[:,:-1], delta[:,-1:]), 1)).sum(axis = 1)
...
def stat_arb_success(stock_test, net):
    ...
    for i in range(int(len(stock_test[0])/10)):
        ...
        for k in range(z):
          ...
          cost += trans_cost * np.abs(stock_test[k][10*i:10*i+10] * np.concatenate([np.array([delta[0]]), delta[1:]-delta[:-1], np.array([delta[-1]])])).sum()
